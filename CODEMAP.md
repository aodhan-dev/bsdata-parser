...
- `harness/oracle.ts` — resolve `WARHOST_DATA_DIR`; `loadBsdata()` + `loadOracle()` + `hasData()`.
...
- `harness/golden-parity.test.ts` — THE acceptance test (red until the rebuild reaches parity;
  auto-skips unless `WARHOST_DATA_DIR` is set).
...
- No data is committed (`.gitignore`); supply it locally via `WARHOST_DATA_DIR`.
